...
func InitializeLedger(dbPath string, initialDifficulty uint32, minerAddr string) *LedgerCore {
	...
	if err != nil {
		panic(fmt.Sprintf("Gagal membuka database: %v", err))
	}
	...
	if !coreLedger.LoadFromDisk() {
		fmt.Println("[DB] Database kosong. Mencetak Genesis Block...")
		coreLedger.SpawnGenesis()
	} else {
		fmt.Println("[DB] Rantai blok berhasil dimuat dari LevelDB!")
	}
...
func (lc *LedgerCore) AddToMempool(tx *core.Transfer) bool {
	...
	if !tx.Verify() {
		fmt.Println("[MEMPOOL] ❌ Tanda tangan transaksi tidak sah!")
		return false
	}
	...
	if !exists || acc.Balance < (tx.Value + tx.Fee) {
		fmt.Println("[MEMPOOL] ❌ Saldo pengirim tidak cukup!")
		return false
	}
	if tx.Nonce != acc.Nonce {
		fmt.Printf("[MEMPOOL] ❌ Nonce tidak sinkron (Expected: %d, Got: %d)\n", acc.Nonce, tx.Nonce)
		return false
	}

	lc.Mempool = append(lc.Mempool, tx)
	fmt.Printf("[MEMPOOL] 📥 Transaksi masuk antrean (ID: %s...)\n", tx.ComputeID()[:12])
	return true
}
	...
	go func() {
		for {
			...
			case <-ticker.C:
				...
				if count > 0 {
					fmt.Printf("\n[NODE] 🔄 Memproses mempool: Mengemas %d transaksi...\n", count)
					lc.MineBlock()
				}
			case <-lc.StopSignal:
				ticker.Stop()
...
func (lc *LedgerCore) MineBlock() {
	...
	fmt.Printf("[MINER] ⛏️ Menambang Blok #%d...\n", newBlock.Index)
	...
	fmt.Println("--------------------------------------------------------------------------------")
	fmt.Printf("[SUKSES] 🚀 Blok #%d Tersimpan di LevelDB! (Nonce: %d, Tx: %d)\n", newBlock.Index, newBlock.Nonce, len(newBlock.Transfers))
	fmt.Println("--------------------------------------------------------------------------------")
}
